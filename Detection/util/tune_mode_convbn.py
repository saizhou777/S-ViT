...
def efficient_conv_bn_eval_forward(
    ...
    weight_coeff = torch.rsqrt(bn.running_var + bn.eps).reshape([-1] + [1] *
...
def efficient_conv_bn_eval_control(
    ...
    '''def forward(x):
        conv_out = conv(x)
        dev = conv_out.device
        if bn.running_mean.device != dev:
            bn.running_mean.data = bn.running_mean.data.to(dev)
            bn.running_var.data = bn.running_var.data.to(dev)
            if bn.bias is not None and bn.bias.device != dev:
                bn.bias.data = bn.bias.data.to(dev)
            if bn.weight is not None and bn.weight.device != dev:
                bn.weight.data = bn.weight.data.to(dev)
        return bn(conv_out)
    return forward  '''               #############################################################
