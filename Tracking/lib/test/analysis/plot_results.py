# 在 lib/test/analysis/plot_results.py 文件开头添加
import matplotlib
matplotlib.use('Agg')
...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    title = plot_opts['title']  # 简化标题，去掉LaTeX格式
    ...
    plt.draw()'''
    fig.savefig('{}/{}_plot.pdf'.format(result_plot_path, plot_type), dpi=300, format='pdf', transparent=True)
    fig.savefig('{}/{}_plot.png'.format(result_plot_path, plot_type), dpi=300, format='png', transparent=True)
    print(f"Saved plot: {result_plot_path}/{plot_type}_plot.pdf")
